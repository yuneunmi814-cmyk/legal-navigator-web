...
export function mountJourney(host,{stage,data,go}){
  ...
  const value=k=>['text','date','evidence'].includes(k)?event[k]:data[k];
  const update=(k,v)=>{data.confirmed=false;if(['text','date','evidence'].includes(k))event[k]=v;else data[k]=v;};
  ...
  const field=(parent,k,label)=>{const l=node('label',label),i=node(['text','questions'].includes(k)?'textarea':'input');i.id='story-'+k;l.htmlFor=i.id;i.value=value(k);i.maxLength=4000;i.oninput=()=>update(k,i.value);parent.append(l,i);return i;};
  ...
  if(stage===4){
    ...
    host.append(optional,node('p','실명·주민등록번호·연락처 대신 ‘본인’, ‘상대방’처럼 적어도 됩니다. 새로고침하면 입력은 사라집니다.'));
    host.lastChild.className='hint';
    host.append(button('정리한 내용 확인하기',()=>{
      if(!event.text.trim()&&!data.intake){document.getElementById('status').textContent='기억나는 일을 한 가지 적어 주세요. 날짜나 자료는 비워 두어도 됩니다.';text.focus();return;}go(5);
    },true));
  }
  if(stage===5){
    ...
    const edits=[];
    for(const [k,label] of fields){const row=node('details');row.className='review-item';row.append(node('summary',label+' · '+(value(k)||'미확인')+' · 수정'));field(row,k,label+' 수정');edits.push(row);host.append(row);}
