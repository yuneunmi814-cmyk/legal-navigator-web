...
export function issues(data){
  ...
  data.events.forEach((event,i)=>{
    if(!event.text.trim())result.push(`${i+1}번 사건의 내용이 비어 있습니다.`);
    if(!event.date.trim())result.push(`${i+1}번 사건의 날짜는 미확인입니다.`);
    if(!event.evidence.trim())result.push(`${i+1}번 사건의 관련 자료는 미확인입니다. 자료가 없다는 뜻은 아닙니다.`);
...
export function draft(data){
  ...
  return ['사건 경위 및 증거 정리 — 상담·작성 준비용 초안',
    ...
    '\n5. 증거·자료 목록',...data.events.map((e,i)=>`${i+1}번 사건 관련: ${value(e.evidence)}`),
