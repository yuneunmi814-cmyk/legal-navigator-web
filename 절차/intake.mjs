...
export function review(host,source=''){
 ...
 const inputs={},candidates=extract(source);
 host.append(el('p','찾은 항목은 후보로 채웠습니다. 후보마다 원문 근거를 확인하세요. 날짜·내 역할은 추정하지 않습니다. 불필요한 개인정보는 지우고, 이름도 필요할 때만 이어 쓰세요.'));
 for(const [key,label,hint] of fields){const row=el('div');row.style.margin='18px 0';const l=el('label',label);l.htmlFor='intake-'+key;const input=el('textarea');input.id=l.htmlFor;input.maxLength=1500;input.rows=2;input.style.cssText='display:block;width:100%;box-sizing:border-box;font:inherit';input.placeholder=hint;inputs[key]=input;row.append(l,input,el('small',hint));const candidate=candidates[key];
  if(candidate?.value){input.value=candidate.value;const evidence=el('button','추출 근거 보기');evidence.type='button';evidence.onclick=()=>{original.open=true;pre.replaceChildren(document.createTextNode(source.slice(0,candidate.start)));const mark=el('mark',source.slice(candidate.start,candidate.end));pre.append(mark,document.createTextNode(source.slice(candidate.end)));mark.scrollIntoView({block:'center'});};row.append(el('p','자동 추출 후보 — 아직 확인되지 않았습니다.'),evidence);}
  else row.append(el('p',candidate?.ambiguous?'서로 다른 값이 여러 개입니다. 해당 문서를 확인해 직접 골라 주세요.':'찾지 못한 항목입니다. 모르면 비워 두세요.'));
  host.append(row);}
